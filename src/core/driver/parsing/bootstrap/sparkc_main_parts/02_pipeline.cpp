...
struct ScopedEnvOverride {
  ...
  bool already_saved(const std::string& name) const {
    return std::find_if(saved_.begin(), saved_.end(), [&](const SavedEntry& entry) {
             return entry.name == name;
           }) != saved_.end();
  }

  void snapshot_if_needed(const std::string& name) {
    if (already_saved(name)) {
      return;
    }
    const auto* current = std::getenv(name.c_str());
    saved_.push_back({name, current ? std::optional<std::string>(current) : std::nullopt});
  }

  void set(const std::string& name, const std::string& value) {
    snapshot_if_needed(name);
...
bool compile_c_source_to_binary(const std::filesystem::path& c_path,
  ...
  {
    const auto target_ld_flags = split_env_flags(std::getenv("SPARK_TARGET_LDFLAGS"));
    for (const auto& flag : target_ld_flags) {
      append_if_missing(native_ld_flags, flag);
    }
  }
