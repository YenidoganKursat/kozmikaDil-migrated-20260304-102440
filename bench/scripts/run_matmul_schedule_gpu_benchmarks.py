...
K_BIN = ROOT_DIR / "k"
PROGRAM_DIR = ROOT_DIR / "bench" / "programs" / "matmul_schedule_gpu"
RESULTS_DIR = ROOT_DIR / "bench" / "results"

sys.path.insert(0, str(SCRIPT_DIR / "matmul_schedule_gpu"))
from definitions import matmul_schedule_gpu_benchmarks  # noqa: E402
from utils import (  # noqa: E402
    collect_timing_samples_repeated,
    compile_c_baseline,
    parse_c_baseline_output,
    parse_matmul_schedule_gpu_output,
    summarize_times,
    within_tolerance,
)
...
def write_results(config, records, comparisons):
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    json_path = RESULTS_DIR / "matmul_schedule_gpu_benchmarks.json"
    csv_path = RESULTS_DIR / "matmul_schedule_gpu_benchmarks.csv"
...
def main():
    ...
    defs = matmul_schedule_gpu_benchmarks(ROOT_DIR)
    records = []
    comparisons = []
    for defn in defs:
    ...
    print(f"matmul_schedule_gpu records: {passed}/{len(records)} passed")
    print(f"matmul_schedule_gpu comparisons: {cmp_passed}/{len(comparisons)} passed")
