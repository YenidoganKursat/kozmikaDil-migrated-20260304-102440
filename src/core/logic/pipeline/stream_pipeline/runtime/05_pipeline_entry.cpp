...
bool try_execute_pipeline_call(const CallExpr& call, Interpreter& self,
  ...
  stream_pipeline::PipelineChain chain;
  if (!stream_pipeline::build_pipeline_chain(call, self, env, chain)) {
    return false;
  }

  if (chain.base_value.kind == Value::Kind::List) {
    out = stream_pipeline::execute_list_pipeline(chain, env);
    return true;
  }
  if (chain.base_value.kind == Value::Kind::Matrix) {
    out = stream_pipeline::execute_matrix_pipeline(chain, env);
    return true;
  }
...
Value pipeline_stats_value(const std::shared_ptr<Environment>& env, const std::string& name) {
  const auto* stats = stream_pipeline::get_pipeline_stats_const(env, name);
