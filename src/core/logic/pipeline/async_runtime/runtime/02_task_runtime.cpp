  ...
  if (timeout_ms.has_value()) {
    ...
    while (future.wait_for(std::chrono::milliseconds(0)) != std::future_status::ready) {
      ...
      if (task.task_value->has_scheduler_queue_hint) {
        assisted = async_runtime::scheduler_assist_one_from_queue(task.task_value->scheduler_queue_hint);
      }
      if (!assisted) {
        assisted = async_runtime::scheduler_assist_one();
      }
      if (!assisted) {
...
Value spawn_task_value(const Value& callee, const std::vector<Value>& args,
  ...
  auto future = async_runtime::scheduler_submit([callee, args, token]() -> Value {
...
Value scheduler_stats_value() {
  const auto stats = async_runtime::scheduler_stats_snapshot();
