"""Language runners for fair integer cross-language benchmark."""
...
def run_kozmika(
    ...
    with tempfile.TemporaryDirectory(prefix=f"kozmika-int-{primitive}-{op_tag}-") as tmp:
        ...
        if mode == "native":
            env.update(
                {
                    ...
                    "-fno-trapping-math -fno-fast-math",
                    "SPARK_LTO": "full",
                }
            )
            run_checked(
            ...
            cmd = ["./k", "run", str(source), "--interpret"]
        return _timed_runs(
            cmd,
            loops,
            lanes,
...
def run_c_like(
    ...
    if shutil.which(compiler) is None:
        return None
    with tempfile.TemporaryDirectory(prefix=f"int-fair-{language}-") as tmp:
        tmpdir = pathlib.Path(tmp)
...
def run_go(
    ...
    if shutil.which("go") is None:
        return None
    with tempfile.TemporaryDirectory(prefix="int-fair-go-") as tmp:
        tmpdir = pathlib.Path(tmp)
...
def run_java(
    ...
    if shutil.which("javac") is None or shutil.which("java") is None:
        return None
    with tempfile.TemporaryDirectory(prefix="int-fair-java-") as tmp:
        tmpdir = pathlib.Path(tmp)
...
def run_csharp(
    ...
    if shutil.which("dotnet") is None:
        return None
    with tempfile.TemporaryDirectory(prefix="int-fair-csharp-") as tmp:
        tmpdir = pathlib.Path(tmp)
