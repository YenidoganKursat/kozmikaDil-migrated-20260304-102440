...
K_BIN = ROOT_DIR / "k"
PROGRAM_DIR = ROOT_DIR / "bench" / "programs" / "hetero_runtime"
RESULTS_DIR = ROOT_DIR / "bench" / "results"

sys.path.insert(0, str(SCRIPT_DIR / "hetero_runtime"))
from definitions import hetero_runtime_benchmarks  # noqa: E402
from utils import (  # noqa: E402
    collect_timing_samples,
    parse_hetero_runtime_output,
    summarize_times,
    try_collect_perf,
    within_tolerance,
)
...
def run_benchmark(defn, args):
    ...
    if status_ok:
        try:
            parsed = parse_hetero_runtime_output(first_output)
...
def write_outputs(records, comparisons, config):
    ...
    json_path = RESULTS_DIR / "hetero_runtime_benchmarks.json"
    csv_path = RESULTS_DIR / "hetero_runtime_benchmarks.csv"
    cmp_csv_path = RESULTS_DIR / "hetero_runtime_benchmarks_comparisons.csv"
...
def main():
    ...
    defs = hetero_runtime_benchmarks(ROOT_DIR)
    ...
    print(f"hetero_runtime benchmarks: {passed}/{len(records)} passed")
    print(f"hetero_runtime comparisons: {cmp_passed}/{len(comparisons)} passed")
