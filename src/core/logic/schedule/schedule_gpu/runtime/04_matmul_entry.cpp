...
Value matrix_from_f64(std::size_t rows, std::size_t cols, std::vector<double> data,
  ...
  long double reduced_sum = 0.0L;
  if (emit_int) {
    for (std::size_t i = 0; i < data.size(); ++i) {
      const auto rounded = std::llround(data[i]);
      out[i].kind = Value::Kind::Int;
      out[i].int_value = rounded;
      reduced_sum += static_cast<long double>(rounded);
    }
  } else {
    for (std::size_t i = 0; i < data.size(); ++i) {
      const auto entry = data[i];
      out[i].kind = Value::Kind::Double;
      out[i].double_value = entry;
      reduced_sum += static_cast<long double>(entry);
    }
  }
  ...
  result.matrix_cache.reduced_sum_version = result.matrix_cache.version;
  result.matrix_cache.reduced_sum_value = static_cast<double>(reduced_sum);
  result.matrix_cache.reduced_sum_is_int = emit_int;
  return result;
}
...
Value matrix_from_f32(std::size_t rows, std::size_t cols, std::vector<float> data) {
  ...
  long double reduced_sum = 0.0L;
  for (std::size_t i = 0; i < data.size(); ++i) {
    const auto entry = static_cast<double>(data[i]);
    dense_f64[i] = entry;
    out[i].kind = Value::Kind::Double;
    out[i].double_value = entry;
    reduced_sum += static_cast<long double>(entry);
  ...
  result.matrix_cache.reduced_sum_version = result.matrix_cache.version;
  result.matrix_cache.reduced_sum_value = static_cast<double>(reduced_sum);
  result.matrix_cache.reduced_sum_is_int = false;
  return result;
}
...
Value run_matmul_sum_impl(Value& lhs, const Value& rhs, bool use_f32) {
  ...
  const bool cache_enabled = stream_pipeline::env_flag_enabled("SPARK_MATMUL_SUM_CACHE", true);
...
Value run_matmul4_sum_impl(Value& a, const Value& b, const Value& c, const Value& d,
  ...
  const bool cache_enabled = stream_pipeline::env_flag_enabled("SPARK_MATMUL4_SUM_CACHE", true);
  std::string cache_key;
  auto& sum_cache = matmul_sum_cache_store();
  if (cache_enabled) {
  ...
  const bool fast_sum_path = stream_pipeline::env_flag_enabled("SPARK_MATMUL4_SUM_FAST", true);
  if (fast_sum_path) {
    if (use_f32) {
...
Value matrix_matmul_value(Value& lhs, const Value& rhs) {
  return matmul_schedule_gpu::matrix_matmul_value(lhs, rhs);
}

Value matrix_matmul_f32_value(Value& lhs, const Value& rhs) {
  return matmul_schedule_gpu::matrix_matmul_f32_value(lhs, rhs);
}

Value matrix_matmul_f64_value(Value& lhs, const Value& rhs) {
  return matmul_schedule_gpu::matrix_matmul_f64_value(lhs, rhs);
}

Value matrix_matmul_sum_value(Value& lhs, const Value& rhs) {
  return matmul_schedule_gpu::matrix_matmul_sum_value(lhs, rhs);
}

Value matrix_matmul_sum_f32_value(Value& lhs, const Value& rhs) {
  return matmul_schedule_gpu::matrix_matmul_sum_f32_value(lhs, rhs);
}

Value matrix_matmul4_sum_value(Value& a, const Value& b, const Value& c, const Value& d) {
  return matmul_schedule_gpu::matrix_matmul4_sum_value(a, b, c, d);
}

Value matrix_matmul4_sum_f32_value(Value& a, const Value& b, const Value& c, const Value& d) {
  return matmul_schedule_gpu::matrix_matmul4_sum_f32_value(a, b, c, d);
}

Value matrix_matmul_add_value(Value& lhs, const Value& rhs, const Value& bias) {
  return matmul_schedule_gpu::matrix_matmul_add_value(lhs, rhs, bias);
}

Value matrix_matmul_axpby_value(Value& lhs, const Value& rhs, const Value& alpha,
                               const Value& beta, const Value& accum) {
  return matmul_schedule_gpu::matrix_matmul_axpby_value(lhs, rhs, alpha, beta, accum);
}

Value matrix_matmul_stats_value(const Value& matrix) {
  return matmul_schedule_gpu::matrix_matmul_stats_value(matrix);
}

Value matrix_matmul_schedule_value(const Value& matrix) {
  return matmul_schedule_gpu::matrix_matmul_schedule_value(matrix);
}
