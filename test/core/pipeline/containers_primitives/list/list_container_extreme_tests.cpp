...
size = len(values)
  ...
  const auto sum_reduce = containers_primitives_test::run_and_get(source, "sum_reduce");
  const auto sum_loop = containers_primitives_test::run_and_get(source, "sum_loop");
  const auto size = containers_primitives_test::run_and_get(source, "size");
...
for v in values:
  ...
  containers_primitives_test::expect_global_int(source, "size", 258);
  containers_primitives_test::expect_global_int(source, "head", -1);
  containers_primitives_test::expect_global_int(source, "tail", 999);
  containers_primitives_test::expect_global_int(source, "probe", 4242);
  containers_primitives_test::expect_global_int(source, "sum_loop", 70511);
}
...
sum_lhs = lhs.reduce_sum()
  ...
  const auto sum_base = containers_primitives_test::run_and_get(source, "sum_base");
  const auto sum_lhs = containers_primitives_test::run_and_get(source, "sum_lhs");
