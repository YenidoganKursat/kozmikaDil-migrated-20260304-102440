    ...
    if (callee_expr->kind == Type::Kind::Builtin) {
      if (call.callee->kind == Expr::Kind::Variable) {
        ...
        if (callee_var == "append") {
          ...
          add_context_reason(
              {.message = "spawn() lowers to async_runtime scheduler task enqueue path", .normalizable = true});
          if (call.args.empty()) {
          ...
          add_context_reason(
              {.message = callee_var + "() lowers to async_runtime work-stealing scheduler", .normalizable = true});
  ...
  if (callee_expr->kind == Type::Kind::Class) {
    for (const auto& arg : call.args) {
      auto arg_type = infer_expr(*arg);
      analyze_expr(*arg, arg_type);
    }
    add_context_reason({.message = "class constructor call", .normalizable = true});
    return callee_expr;
  }
...
void TypeChecker::check_stmt(const Stmt& stmt) {
  switch (stmt.kind) {
    ...
    case Stmt::Kind::Assign: {
      ...
      } else if (assign.target->kind == Expr::Kind::Attribute) {
        auto target_type = infer_lvalue_type(*assign.target);
        if (target_type->kind == Type::Kind::Error) {
          break;
        }
        if (!is_assignable(*rhs, *target_type) && target_type->kind != Type::Kind::Unknown) {
          add_error("cannot assign " + type_to_string(*rhs) + " to attribute target of type " +
                    type_to_string(*target_type));
          add_context_reason({.message = "attribute assignment type mismatch", .normalizable = false});
        }
      } else {
        add_error("invalid assignment target");
      }
    ...
    case Stmt::Kind::FunctionDef: {
      ...
      if (fn.is_async) {
        add_context_reason(
            {.message = "async function lowers to state-machine task frame (async_runtime runtime path)",
             .normalizable = true});
      }
