...
namespace matmul_schedule_gpu_test {
...
c = a.matmul_f64(b)
  ...
  assert(dump.find("matmul eligible for matmul_schedule_gpu kernel scheduling path") != std::string::npos);
}

}  // namespace

void run_matmul_schedule_gpu_analyze_tests() {
