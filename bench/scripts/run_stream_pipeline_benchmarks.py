...
K_BIN = ROOT_DIR / "k"
PROGRAM_DIR = ROOT_DIR / "bench" / "programs" / "stream_pipeline"
RESULTS_DIR = ROOT_DIR / "bench" / "results"

sys.path.insert(0, str(SCRIPT_DIR / "stream_pipeline"))
from definitions import stream_pipeline_benchmarks  # noqa: E402
from utils import (  # noqa: E402
    collect_timing_samples,
    parse_stream_pipeline_output,
    summarize_times,
    within_tolerance,
)
...
def run_mode(defn, mode_name, args):
    ...
    if status_ok:
        try:
            parsed = parse_stream_pipeline_output(first_output)
...
def write_results(config, records, comparisons):
    ...
    json_path = RESULTS_DIR / "stream_pipeline_benchmarks.json"
    csv_path = RESULTS_DIR / "stream_pipeline_benchmarks.csv"
...
def main():
    ...
    defs = stream_pipeline_benchmarks(ROOT_DIR)
    records = []
    comparisons = []
    for defn in defs:
    ...
    print(f"stream_pipeline records: {passed}/{len(records)} passed")
    print(f"stream_pipeline comparisons: {cmp_passed}/{len(comparisons)} passed")
