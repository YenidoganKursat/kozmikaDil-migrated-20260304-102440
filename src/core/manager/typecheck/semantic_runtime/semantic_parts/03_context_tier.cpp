...
TypePtr TypeChecker::infer_lvalue_type(const Expr& target) {
  ...
  if (target.kind == Expr::Kind::Attribute) {
    const auto& attr = static_cast<const AttributeExpr&>(target);
    auto owner = infer_expr(*attr.target);
    if (owner->kind == Type::Kind::Unknown || owner->kind == Type::Kind::Any ||
        owner->kind == Type::Kind::Class) {
      return unknown_type();
    }
    add_error("attribute assignment requires object/class receiver");
    return error_type();
  }
...
TypePtr TypeChecker::check_call(const CallExpr& call) {
  ...
  const auto is_sendable_for_async_runtime = [](const Type& type) {
    switch (type.kind) {
      case Type::Kind::Nil:
      case Type::Kind::Int:
  ...
  const auto report_non_sendable_capture = [&](const std::string& site, std::size_t arg_index,
    ...
    add_context_reason({.message = "Cannot lower to async_runtime fast path: non-sendable capture (" + site + ")",
                        .normalizable = false});
  };
  ...
  if (call.callee->kind == Expr::Kind::Attribute) {
    ...
    if (attribute.attribute == "matmul" || attribute.attribute == "matmul_f32" ||
      ...
      if (target_type->list_element &&
        ...
        add_context_reason({.message = "matmul eligible for matmul_schedule_gpu kernel scheduling path",
                            .normalizable = true});
      }
    ...
    if (attribute.attribute == "spawn" || attribute.attribute == "join_all" ||
      ...
      if (attribute.attribute == "spawn") {
        ...
        if (call.args[0]->kind != Expr::Kind::Variable) {
          ...
          add_context_reason({.message = "task_group.spawn() eligible for async_runtime scheduler enqueue path",
                              .normalizable = true});
        }
        for (std::size_t i = 1; i < call.args.size(); ++i) {
          const auto arg_type = infer_expr(*call.args[i]);
          analyze_expr(*call.args[i], arg_type);
          if (arg_type->kind != Type::Kind::Error && !is_sendable_for_async_runtime(*arg_type)) {
    ...
    if (attribute.attribute == "join" || attribute.attribute == "cancel") {
      ...
      if (attribute.attribute == "join") {
        add_context_reason({.message = "task.join() resumes async result in async_runtime runtime",
                            .normalizable = true});
    ...
    if (attribute.attribute == "send" || attribute.attribute == "recv" ||
      ...
      if (attribute.attribute == "recv") {
        ...
        add_context_reason({.message = "channel recv path integrates with async_runtime async wait semantics",
                            .normalizable = true});
