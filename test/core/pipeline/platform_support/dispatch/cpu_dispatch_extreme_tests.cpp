...
void test_unknown_arch_falls_back_to_scalar_dispatch() {
  ...
  assert(spark::matmul_variant_tag(false) == "scalar");
  assert(spark::recommended_vector_width(false) == 2);
  assert(spark::recommended_vector_width(true) == 4);
}
...
void test_feature_list_dedup_and_separator_parsing() {
  ...
  assert(spark::recommended_vector_width(true) == 16);
}
...
void test_f32_dispatch_width_for_all_supported_arch_overrides() {
  ...
  assert(spark::matmul_variant_tag(true) == "arm_sve2");
  assert(spark::recommended_vector_width(true) == 8);

  arch.set("riscv64");
  features.set("rv64,rvv");
  assert(spark::matmul_variant_tag(true) == "riscv_rvv");
  assert(spark::recommended_vector_width(true) == 8);
}

}  // namespace

void run_platform_support_dispatch_extreme_tests() {
