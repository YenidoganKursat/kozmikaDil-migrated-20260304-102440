...
Value evaluate_case_attribute(const AttributeExpr& attribute, Interpreter& self,
  ...
  Value receiver_local = Value::nil();
  Value* receiver = nullptr;
  if (target_is_variable) {
    receiver = env ? env->get_ptr(target_name) : nullptr;
  } else if (captured_target.has_value()) {
    receiver_local = *captured_target;
    receiver = &receiver_local;
  }

  if (receiver && receiver->kind == Value::Kind::Object && receiver->object_value) {
    auto field_it = receiver->object_value->fields.find(attribute.attribute);
    if (field_it != receiver->object_value->fields.end()) {
      return field_it->second;
    }
    if (receiver->object_value->class_def) {
      auto method_it = receiver->object_value->class_def->methods.find(attribute.attribute);
      if (method_it != receiver->object_value->class_def->methods.end()) {
        Value method = method_it->second;
        Value bound_self = *receiver;
        return Value::builtin(
            attribute.attribute,
            [method, bound_self](const std::vector<Value>& args) -> Value {
              std::vector<Value> call_args;
              call_args.reserve(args.size() + 1);
              call_args.push_back(bound_self);
              for (const auto& arg : args) {
                call_args.push_back(arg);
              }
              if (method.kind == Value::Kind::Function && method.function_value &&
                  method.function_value->is_async) {
                return spawn_task_value(method, call_args);
              }
              return invoke_callable_sync(method, call_args);
            });
      }
    }
    throw EvalException("attribute not found on object: " + attribute.attribute);
  }

  if (receiver && receiver->kind == Value::Kind::Class && receiver->class_value) {
    auto field_it = receiver->class_value->field_defaults.find(attribute.attribute);
    if (field_it != receiver->class_value->field_defaults.end()) {
      return field_it->second;
    }
    auto method_it = receiver->class_value->methods.find(attribute.attribute);
    if (method_it != receiver->class_value->methods.end()) {
      return method_it->second;
    }
    throw EvalException("attribute not found on class: " + attribute.attribute);
  }
