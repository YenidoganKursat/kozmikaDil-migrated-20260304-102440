...
bool CodeGenerator::infer_top_level(const Program& program) {
  for (const auto& stmt : program.body) {
    if (stmt->kind == Stmt::Kind::FunctionDef) {
      ...
      if (fn.is_async) {
        add_error("async function definitions are unsupported in scalar_runtime codegen: " + fn.name);
        return false;
      }
    ...
    if (stmt->kind == Stmt::Kind::ClassDef) {
      ...
      add_error("class definitions are unsupported in scalar_runtime codegen: " + cls.name);
      return false;
    }
  }
...
bool CodeGenerator::emit_range_loop_setup(const Expr& iterable, FunctionContext& ctx, Code& start_code,
  ...
  if (call->args.size() == 1) {
    ...
    if (!is_integer_constant_expr(*call->args[2], step_value)) {
      add_error("range step must be compile-time integer constant in scalar_runtime");
      return false;
    }
  }
  ...
  if (start_code.kind != ScalarKind::Int || stop_code.kind != ScalarKind::Int) {
    add_error("range bounds must be integer expressions in scalar_runtime");
    return false;
  }
  if (step_value == 0) {
...
bool CodeGenerator::emit_for_statement(const ForStmt& for_stmt, FunctionContext& ctx) {
  if (for_stmt.is_async) {
    add_error("async for is unsupported in scalar_runtime codegen");
    return false;
  }
