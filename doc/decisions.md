...
- Implementation rule:
  - `scripts/core/pipeline/phase10/multiarch_build.py` emits `bench/results/phase10_multiarch.json`,
...
- Implementation rule:
  - catalog source: `scripts/core/pipeline/phase10/target_catalog.py`,
  - orchestrator: `scripts/core/pipeline/phase10/multiarch_build.py` supports:
    - `--preset`
...
- Implementation rule:
  ...
  - the guard executes `scripts/core/pipeline/phase10/platform_matrix.py` with market-wide CPU/GPU options and enforces minimum catalog coverage.
  - CI workflow adds:
...
- Implementation rule:
  - smoke script: `scripts/core/pipeline/phase10/gpu_smoke_matrix.py`
  - supports:
...
- Implementation rule:
  - script: `scripts/core/pipeline/phase10/gpu_backend_perf.py`
  - metrics per backend:
...
- Implementation rule:
  ...
  - runtime perf script `scripts/core/pipeline/phase10/gpu_backend_runtime_perf.py` records `requested_backend -> effective_backend` and latency.
