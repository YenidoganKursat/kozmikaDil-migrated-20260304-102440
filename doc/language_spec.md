...
- Class definitions:
  - `class Name: ...`
  - `class Name(slots): ...` (sabit alanlar)
  - `class Name(open): ...` (dinamik alan eklenebilir)
...
- Aggregate/runtime values:
  - `list[T]` / `list[Any]`
  - `matrix[T]` / `matrix[Any]`
  - `class` and `object` runtime values
...
## 4.1 Object Model (Java-benzeri)
- Constructor call: `u = User(...)`
- Field read: `u.age`
- Field write: `u.age = 31`
- Method call: `u.birthday()`
- `init(self, ...)` veya `__init__(self, ...)` constructor sırasında otomatik çağrılır.
- `slots` class: sadece tanımlı alanlara yazılır.
- `open` class: çalışma anında yeni alan yazılabilir.
