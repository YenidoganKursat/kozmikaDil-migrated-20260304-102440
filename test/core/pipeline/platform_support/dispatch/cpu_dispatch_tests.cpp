...
void test_cpu_report_basic_shape() {
  ...
  assert(report.find("matmul_variant_f64=") != std::string::npos);
  assert(report.find("matmul_vector_width_f64=") != std::string::npos);
}
...
void test_x86_dispatch_override() {
  ...
  assert(spark::matmul_variant_tag(false) == "x86_avx2");
  assert(spark::recommended_vector_width(false) == 4);

  features.set("sse2,avx2,avx512f");
  assert(spark::matmul_variant_tag(false) == "x86_avx512");
  assert(spark::recommended_vector_width(false) == 8);
}
...
void test_arm_dispatch_override() {
  ...
  arch.set("aarch64");
  features.set("neon");
  assert(spark::matmul_variant_tag(false) == "arm_neon");
  assert(spark::recommended_vector_width(false) == 2);

  features.set("neon,sve2");
  assert(spark::matmul_variant_tag(false) == "arm_sve2");
  assert(spark::recommended_vector_width(false) == 4);
}
...
void test_riscv_dispatch_override() {
  ...
  assert(spark::matmul_variant_tag(false) == "riscv_rvv");
  assert(spark::recommended_vector_width(false) == 4);

  features.set("rv64");
  assert(spark::matmul_variant_tag(false) == "riscv_baseline");
}

}  // namespace

void run_platform_support_dispatch_tests() {
