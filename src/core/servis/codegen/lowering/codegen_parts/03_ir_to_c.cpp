IRToCResult IRToCGenerator::translate(const std::string& ir, const IRToCOptions& options) {
  ...
  generated << "#define __SPARK_STRING_SMALL_CAP 23\n";
  generated << "typedef struct { char* data; i64 bytes; i64 codepoints; unsigned char storage; char small[__SPARK_STRING_SMALL_CAP + 1]; } __spark_string;\n\n";
  generated << R"(#define __SPARK_STRING_STORAGE_HEAP 0u
#define __SPARK_STRING_STORAGE_SMALL 1u
#define __SPARK_STRING_STORAGE_BORROWED 2u
static SPARK_FORCE_INLINE const char* __spark_string_data_const(const __spark_string* s) {
  if (!s || s->bytes <= 0) return NULL;
  if (s->storage == __SPARK_STRING_STORAGE_SMALL) return s->small;
  return s->data;
}
static SPARK_FORCE_INLINE char* __spark_string_data_mut(__spark_string* s) {
  if (!s || s->bytes <= 0) return NULL;
  if (s->storage == __SPARK_STRING_STORAGE_SMALL) return s->small;
  return s->data;
}
static SPARK_FORCE_INLINE i64 __spark_utf8_codepoint_count(const char* data, i64 bytes) {
...
static SPARK_FORCE_INLINE bool __spark_utf8_is_ascii(const char* data, i64 bytes) {
  if (!data || bytes <= 0) return true;
  for (i64 i = 0; i < bytes; ++i) {
    if (((unsigned char)data[i]) & 0x80u) return false;
  }
  return true;
}
static SPARK_FORCE_INLINE i64 __spark_utf8_codepoint_to_byte(const __spark_string* s, i64 cp_index) {
  const char* data = __spark_string_data_const(s);
  if (!s || !data || s->bytes <= 0 || cp_index <= 0) return 0;
  i64 cp = 0;
...
static SPARK_FORCE_INLINE char* __spark_string_alloc_raw(size_t bytes) {
  if (bytes == 0u) return NULL;
  static char* arena = NULL;
  static size_t arena_cap = 0u;
  static size_t arena_pos = 0u;
  const size_t align = sizeof(void*);
  size_t aligned_pos = (arena_pos + (align - 1u)) & ~(align - 1u);
  if (aligned_pos > ((size_t)-1) - bytes) return NULL;
  const size_t required = aligned_pos + bytes;
  if (required > arena_cap) {
    size_t new_cap = (arena_cap > 0u) ? arena_cap : (1u << 20);
    while (new_cap < required) {
      const size_t grown = new_cap + (new_cap >> 1u);
      if (grown <= new_cap) {
        new_cap = required;
        break;
      }
    ...
    char* resized = (char*)realloc(arena, new_cap);
    if (!resized) {
...
static SPARK_FORCE_INLINE __spark_string __spark_string_empty(void) {
  ...
  out.bytes = 0;
  out.codepoints = 0;
  out.storage = __SPARK_STRING_STORAGE_SMALL;
  out.small[0] = '\0';
  return out;
}
static SPARK_FORCE_INLINE __spark_string __spark_string_from_bytes(const char* data, i64 bytes) {
  if (!data || bytes <= 0) {
    return __spark_string_empty();
  }
  __spark_string out;
  const bool ascii = __spark_utf8_is_ascii(data, bytes);
  if (bytes <= (i64)__SPARK_STRING_SMALL_CAP) {
    out.data = NULL;
    out.storage = __SPARK_STRING_STORAGE_SMALL;
    memcpy(out.small, data, (size_t)bytes);
    out.small[bytes] = '\0';
    out.bytes = bytes;
    out.codepoints = ascii ? bytes : __spark_utf8_codepoint_count(data, bytes);
    return out;
  }
  out.data = __spark_string_alloc_raw((size_t)bytes + 1u);
  if (!out.data) {
    out.bytes = 0;
    out.codepoints = 0;
    out.storage = __SPARK_STRING_STORAGE_SMALL;
  ...
  out.storage = __SPARK_STRING_STORAGE_HEAP;
...
static SPARK_FORCE_INLINE __spark_string __spark_string_from_utf8(const char* text) {
  if (!text) return __spark_string_empty();
  const i64 bytes = (i64)strlen(text);
  if (bytes <= 0) return __spark_string_empty();
  __spark_string out;
  out.data = (char*)text;
  out.bytes = bytes;
  out.codepoints = __spark_utf8_is_ascii(text, bytes) ? bytes : __spark_utf8_codepoint_count(text, bytes);
  out.storage = __SPARK_STRING_STORAGE_BORROWED;
  out.small[0] = '\0';
  return out;
}
static SPARK_FORCE_INLINE __spark_string __spark_string_from_ascii_literal(const char* text, i64 bytes) {
  if (!text || bytes <= 0) return __spark_string_empty();
  __spark_string out;
  out.data = (char*)text;
  out.bytes = bytes;
  out.codepoints = bytes;
  out.storage = __SPARK_STRING_STORAGE_BORROWED;
  out.small[0] = '\0';
  return out;
}
static SPARK_FORCE_INLINE char* __spark_u64_to_dec_rev(unsigned long long value, char* end) {
  ...
  while (u >= 100ULL) {
    const unsigned pair = (unsigned)((u % 100ULL) * 2ULL);
    u /= 100ULL;
    *--end = digits[pair + 1u];
    *--end = digits[pair];
  }
  if (u < 10ULL) {
    *--end = (char)('0' + (unsigned)u);
  } else {
    const unsigned pair = (unsigned)(u * 2ULL);
    *--end = digits[pair + 1u];
    *--end = digits[pair];
  }
  return end;
}
static SPARK_FORCE_INLINE __spark_string __spark_string_from_i64(i64 value) {
  char buffer[32];
  ...
  char* cur = __spark_u64_to_dec_rev(u, end);
  if (neg) {
    *--cur = '-';
  }
  __spark_string out;
  const i64 bytes = (i64)(end - cur);
  out.data = NULL;
  out.bytes = bytes;
  out.codepoints = bytes;
  out.storage = __SPARK_STRING_STORAGE_SMALL;
  memcpy(out.small, cur, (size_t)bytes);
  out.small[bytes] = '\0';
  return out;
}
static SPARK_FORCE_INLINE __spark_string __spark_string_from_ascii1_i64_prefix(i64 ascii_code, i64 value) {
  const char prefix = (char)(ascii_code & 0x7F);
  char digits[32];
  ...
  const i64 num_bytes = (i64)(end - cur);
  const i64 out_bytes = 1 + num_bytes;
  __spark_string out;
  out.data = NULL;
  out.storage = __SPARK_STRING_STORAGE_SMALL;
  out.bytes = out_bytes;
  out.codepoints = out_bytes;
  out.small[0] = prefix;
  memcpy(out.small + 1, cur, (size_t)num_bytes);
  out.small[out_bytes] = '\0';
  return out;
}
static SPARK_FORCE_INLINE __spark_string __spark_string_from_ascii1_i64_suffix(i64 ascii_code, i64 value) {
  const char suffix = (char)(ascii_code & 0x7F);
  char digits[32];
  char* end = digits + sizeof(digits);
  unsigned long long u = 0ULL;
  bool neg = (value < 0);
  ...
  const i64 num_bytes = (i64)(end - cur);
  const i64 out_bytes = num_bytes + 1;
  __spark_string out;
  out.data = NULL;
  out.storage = __SPARK_STRING_STORAGE_SMALL;
  out.bytes = out_bytes;
  out.codepoints = out_bytes;
  memcpy(out.small, cur, (size_t)num_bytes);
  out.small[num_bytes] = suffix;
  out.small[out_bytes] = '\0';
  return out;
}
...
static i64 __spark_string_utf16_len(__spark_string value) {
  const char* data = __spark_string_data_const(&value);
  if (!data || value.bytes <= 0) return 0;
  i64 units = 0;
  for (i64 i = 0; i < value.bytes;) {
    const unsigned char c0 = (unsigned char)data[i];
    uint32_t cp = 0xFFFDu;
    i64 advance = 1;
    if ((c0 & 0x80u) == 0u) {
      cp = c0;
...
static SPARK_FORCE_INLINE __spark_string __spark_string_concat(__spark_string lhs, __spark_string rhs) {
  const i64 left_bytes = lhs.bytes > 0 ? lhs.bytes : 0;
  const i64 right_bytes = rhs.bytes > 0 ? rhs.bytes : 0;
  if (left_bytes == 0) return rhs;
  if (right_bytes == 0) return lhs;
  const char* lhs_data = __spark_string_data_const(&lhs);
  const char* rhs_data = __spark_string_data_const(&rhs);
  const i64 out_bytes = left_bytes + right_bytes;
  if (out_bytes <= 0) return __spark_string_empty();
  __spark_string out;
  if (out_bytes <= (i64)__SPARK_STRING_SMALL_CAP) {
    out.data = NULL;
    out.storage = __SPARK_STRING_STORAGE_SMALL;
    if (left_bytes > 0 && lhs_data) memcpy(out.small, lhs_data, (size_t)left_bytes);
    if (right_bytes > 0 && rhs_data) memcpy(out.small + left_bytes, rhs_data, (size_t)right_bytes);
    out.small[out_bytes] = '\0';
    out.bytes = out_bytes;
    out.codepoints = (lhs.codepoints > 0 ? lhs.codepoints : 0) + (rhs.codepoints > 0 ? rhs.codepoints : 0);
    return out;
  }
  out.data = __spark_string_alloc_raw((size_t)out_bytes + 1u);
  if (!out.data) {
    out.bytes = 0;
    out.codepoints = 0;
    out.storage = __SPARK_STRING_STORAGE_SMALL;
  ...
  out.storage = __SPARK_STRING_STORAGE_HEAP;
  if (left_bytes > 0 && lhs_data) memcpy(out.data, lhs_data, (size_t)left_bytes);
  if (right_bytes > 0 && rhs_data) memcpy(out.data + left_bytes, rhs_data, (size_t)right_bytes);
  out.data[out_bytes] = '\0';
  out.bytes = out_bytes;
  out.codepoints = (lhs.codepoints > 0 ? lhs.codepoints : 0) + (rhs.codepoints > 0 ? rhs.codepoints : 0);
  return out;
}
static SPARK_FORCE_INLINE i64 __spark_string_cmp(__spark_string lhs, __spark_string rhs) {
  ...
  int cmp = 0;
  if (common > 0 && lhs_data && rhs_data) {
    cmp = memcmp(lhs_data, rhs_data, (size_t)common);
  }
  if (cmp < 0) return -1;
  if (cmp > 0) return 1;
  if (lhs_bytes < rhs_bytes) return -1;
...
static SPARK_FORCE_INLINE bool __spark_string_eq(__spark_string lhs, __spark_string rhs) {
  const i64 lhs_bytes = lhs.bytes > 0 ? lhs.bytes : 0;
  const i64 rhs_bytes = rhs.bytes > 0 ? rhs.bytes : 0;
  if (lhs_bytes != rhs_bytes) return false;
  if (lhs_bytes == 0) return true;
  const char* lhs_data = __spark_string_data_const(&lhs);
  const char* rhs_data = __spark_string_data_const(&rhs);
  if (!lhs_data || !rhs_data) return false;
  if (lhs_data == rhs_data) return true;
  return memcmp(lhs_data, rhs_data, (size_t)lhs_bytes) == 0;
}
static __spark_string __spark_string_index(__spark_string value, i64 index) {
  const char* data = __spark_string_data_const(&value);
  if (!data || value.codepoints <= 0) return __spark_string_empty();
  ...
  return __spark_string_from_bytes(data + start, stop - start);
}
static __spark_string __spark_string_slice(__spark_string value, i64 start, i64 stop, i64 step) {
  const char* data = __spark_string_data_const(&value);
  if (!data || value.codepoints <= 0 || step == 0) return __spark_string_empty();
  i64 s = start;
  i64 e = stop;
  ...
  if (step == 1) {
    ...
    return __spark_string_from_bytes(data + b0, b1 - b0);
  }
  i64 picked_bytes = 0;
  i64 picked_cp = 0;
  ...
  __spark_string out;
  if (picked_bytes <= (i64)__SPARK_STRING_SMALL_CAP) {
    out.data = NULL;
    out.storage = __SPARK_STRING_STORAGE_SMALL;
  } else {
    out.data = __spark_string_alloc_raw((size_t)picked_bytes + 1u);
    if (!out.data) {
      out.bytes = 0;
      out.codepoints = 0;
      out.storage = __SPARK_STRING_STORAGE_SMALL;
  ...
  char* out_data = __spark_string_data_mut(&out);
  i64 cursor = 0;
  for (i64 cp = s; (step > 0) ? (cp < e) : (cp > e); cp += step) {
    if (cp < 0 || cp >= value.codepoints) continue;
    const i64 b0 = __spark_utf8_codepoint_to_byte(&value, cp);
    ...
    if (b1 > b0) {
      ...
      memcpy(out_data + cursor, data + b0, (size_t)width);
      cursor += width;
    }
  }
  out_data[cursor] = '\0';
...
static void __spark_print_str(__spark_string value) {
  const char* data = __spark_string_data_const(&value);
  if (data && value.bytes > 0) {
    fwrite(data, 1u, (size_t)value.bytes, stdout);
  }
  fputc('\n', stdout);
}
)";
