...
bool should_intercept_chain(const PipelineChain& chain) {
  ...
  if (chain.uses_stream_pipeline_only_stage) {
    return true;
  }
  // Keep legacy behavior for single-stage hetero_runtime methods.
...
bool build_pipeline_chain(const CallExpr& call, Interpreter& self,
  ...
  for (const auto* stage_call : reversed_calls) {
    ...
    if (is_stream_pipeline_only_stage(stage.kind)) {
      chain.uses_stream_pipeline_only_stage = true;
    }
