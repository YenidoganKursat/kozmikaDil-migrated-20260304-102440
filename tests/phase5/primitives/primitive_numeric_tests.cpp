...
void run_int_numeric_invariants(spark::Interpreter& interpreter, std::size_t samples) {
  ...
  std::uniform_int_distribution<int> exp_dist(0, 4);
  ...
  for (std::size_t i = 0; i < samples; ++i) {
    ...
    const auto vp = spark::Value::int_value_of(p);
    ...
    const double pow_xp = eval_number(interpreter, Op::Pow, vx, vp);
...
void run_double_numeric_invariants(spark::Interpreter& interpreter, std::size_t samples) {
  ...
  for (std::size_t i = 0; i < samples; ++i) {
    ...
    const auto vp = spark::Value::double_value_of(static_cast<double>(p));
    ...
    assert(std::isfinite(div_xy));
    assert(std::isfinite(mod_xy));
    assert(!std::isnan(pow_xp));  // overflow to inf is allowed; NaN is not for these samples.
    ...
    assert(std::fabs(mod_xy) < std::fabs(y) + 1e-9);
  }
}
...
void run_edge_case_sanity(spark::Interpreter& interpreter) {
  ...
  assert(eval_number(interpreter, Op::Pow, i0, i0) == 1.0);
  assert(eval_number(interpreter, Op::Pow, i1, im1) == 1.0);
  ...
  const auto dm1 = spark::Value::double_value_of(-1.0);

  assert(eval_number(interpreter, Op::Pow, d0, d0) == 1.0);
  assert(eval_number(interpreter, Op::Pow, d1, dm1) == 1.0);
  assert(eval_number(interpreter, Op::Mod, dm1, d1) == 0.0);
}
