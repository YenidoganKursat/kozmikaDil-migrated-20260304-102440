...
BlasSymbols& blas_symbols() {
  static BlasSymbols symbols = [] {
    ...
    if (const auto* forced_lib = std::getenv("SPARK_MATMUL_BLAS_LIB")) {
      if (*forced_lib != '\0') {
        libs.insert(libs.begin(), forced_lib);
      }
    }
    auto try_bind_from_handle = [&](void* handle) -> bool {
      if (!handle) {
        return false;
      }
    ...
    if (try_bind_from_handle(dlopen(nullptr, RTLD_LAZY | RTLD_LOCAL))) {
      return out;
    }
    ...
    for (const auto* lib : libs) {
      ...
      if (!try_bind_from_handle(handle)) {
        dlclose(handle);
        continue;
      }
      return out;
...
void maybe_apply_tuned_file(MatmulSchedule& schedule) {
  const bool use_tuned = spark::stream_pipeline::env_flag_enabled("SPARK_MATMUL_USE_TUNED", true);
...
MatmulSchedule resolve_schedule(const MatmulKernelIR& ir) {
  MatmulSchedule schedule;
  const auto variant = spark::matmul_variant_tag(ir.use_f32);
  schedule.vector_width = spark::recommended_vector_width(ir.use_f32);
  ...
  if (!spark::stream_pipeline::env_flag_enabled("SPARK_MATMUL_DISABLE_DIM_ADAPTIVE", false)) {
    if (!ir.use_f32) {
      if (max_dim <= 160) {
        schedule.tile_m = 96;
        schedule.tile_n = 96;
  ...
  if (backend == "own") {
    ...
    // GPU backend names are accepted as portable requests. Route adaptively to
    // host backend so small/latency-oriented workloads avoid BLAS overhead.
    ...
    const bool force_blas =
        spark::stream_pipeline::env_flag_enabled("SPARK_MATMUL_GPU_FORCE_BLAS", false);
    const bool prefer_blas_by_size = force_blas || large_problem || small_problem;
    if (prefer_blas_by_size && has_blas_backend()) {
      schedule.backend = MatmulBackend::Blas;
      schedule.source = force_blas ? "env_gpu_" + backend + "_force_blas"
                                   : (large_problem ? "env_gpu_" + backend + "_auto_blas_large"
                                                    : "env_gpu_" + backend + "_auto_blas_small");
    } else {
      schedule.backend = MatmulBackend::Own;
      schedule.source = "env_gpu_" + backend + "_auto_own";
    }
  } else {
    const bool respect_tuned_backend =
        spark::stream_pipeline::env_flag_enabled("SPARK_MATMUL_AUTO_RESPECT_TUNED_BACKEND", false);
