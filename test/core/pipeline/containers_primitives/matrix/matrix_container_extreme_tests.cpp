...
while r < rows:
  ...
  const auto expected = containers_primitives_test::run_and_get(source, "expected");
...
block = tt[0:2, 1:3]
  ...
  containers_primitives_test::expect_global_int(source, "probe", 12);
  containers_primitives_test::expect_global_list(source, "row", {5, 6, 7, 8});
  containers_primitives_test::expect_global_list(source, "col", {3, 7, 11});
  containers_primitives_test::expect_global_matrix(source, "block", 2, 2, {2, 3, 6, 7});
}
