...
void test_tier_dump_mentions_async_runtime_scheduler_path() {
  constexpr auto source = R"(
def add(a, b):
  return a + b
...
out = par_reduce(xs, 0, add)
  ...
  assert(tiers.find("async_runtime work-stealing scheduler") != std::string::npos);
}
...
void run_async_runtime_safety_diagnostics_tests() {
  ...
  test_tier_dump_mentions_async_runtime_scheduler_path();
