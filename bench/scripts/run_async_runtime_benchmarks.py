...
K_BIN = ROOT_DIR / "k"
PROGRAM_DIR = ROOT_DIR / "bench" / "programs" / "async_runtime"
RESULTS_DIR = ROOT_DIR / "bench" / "results"

sys.path.insert(0, str(SCRIPT_DIR / "async_runtime"))
from definitions import async_runtime_chunk_sweep, async_runtime_thread_sweep  # noqa: E402
...
def run_parallel_scaling(args):
    ...
    threads = [t for t in async_runtime_thread_sweep() if t <= cpu]
    if 1 not in threads:
        threads = [1] + threads
    runs: List[Dict] = []
    checksum_ref = None
...
def run_chunk_sweep(args):
    ...
    best_chunk = 0
    best_time = 0.0
    for chunk in async_runtime_chunk_sweep():
...
def write_results(config, spawn_record, channel_record, scaling_record, chunk_record):
    ...
    json_path = RESULTS_DIR / "async_runtime_benchmarks.json"
    csv_path = RESULTS_DIR / "async_runtime_benchmarks.csv"
...
def main():
    ...
    print(f"async_runtime sections: {passed_sections}/{total_sections} passed")
